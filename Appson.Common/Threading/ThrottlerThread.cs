...
namespace Appson.Common.Threading
{
    ...
    public class ThrottlerThread
    {
        ...
        public ThrottlerState State
        {
            get
            {
                if (_started && !_stopped && !_paused)
                {
                    return ThrottlerState.Started;
                }
                if (_started && !_stopped && _paused)
                {
                    return ThrottlerState.Paused;
                }
                if (!_started && _stopped)
                {
                    return ThrottlerState.Stopped;
                }
                return ThrottlerState.Unkown;
            }
        }
        ...
        private void ValidateOptions()
        {
            ...
            if (_options.RatePerSecond > 500000d) // Larger than 500k per second
            {
                throw new ArgumentException(
                    $"{nameof(_options.RatePerSecond)} larger than 500k/s is not supported because of %2.5+ inherent error in calculations.");
            }
            ...
            {
                throw new ArgumentException(
                    $"{nameof(_options.InitialQuota)} cannot be larger than {nameof(_options.MaxBurstSize)}.");
            }
            ...
            {
                throw new ArgumentException(
                    $"The specified {nameof(_options.RatePerSecond)} is not achievable given the specified {nameof(_options.MaxBurstSize)} and {nameof(_options.DelayMillis)}. " +
