...
namespace Appson.Common.General.Text
{
    public static class StringExtensions
	{
		...
		public static string FormatIfNotNull(this string format, params object[] formatParams)
		{
			...
			return Format(format, formatParams);
		}

		public static string FormatIfNotEmpty(this string format, params string[] formatParams)
		{
			if (formatParams.Any(IsNullOrWhiteSpace))
				return null;

			return Format(format, formatParams.Cast<object>().ToArray());
		}
		...
		{
			return IsNullOrWhiteSpace(str) ? emptyValue : f(str);
		}

		public static TResult IfNotEmpty<TResult>(this string str, Func<string, TResult> f)
		{
			return IsNullOrWhiteSpace(str) ? default(TResult) : f(str);
		}
		...
		{
			return source?.Where(s => !IsNullOrEmpty(s));
		}
