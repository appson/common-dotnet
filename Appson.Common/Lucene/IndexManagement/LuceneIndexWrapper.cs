...
namespace Appson.Common.Lucene.IndexManagement
{
    internal class LuceneIndexWrapper
	{
		...
		public bool HasErrors => !_searcherErrors.IsEmpty || !_writerErrors.IsEmpty;

	    public List<Exception> WriterErrors => _writerErrors.ToList();

	    public List<Exception> SearcherErrors => _searcherErrors.ToList();

	    public List<Exception> AllErrors => _searcherErrors.Concat(_writerErrors).ToList();
